...
class ClientDisputePage extends StatefulWidget {
  final String jobId;
  ...
  @override
  State<ClientDisputePage> createState() => _ClientDisputePageState();
}

class _ClientDisputePageState extends State<ClientDisputePage> {
  ...
  List<Map<String, dynamic>> _parsePhotos(dynamic value) {
    ...
    try {
      ...
      // algumas vezes pode vir como String json, mas normalmente vem como List
      return [];
    } catch (_) {
      return [];
    }
  ...
  bool _isClientPhotoUrl(String url) {
    ...
    return true; // legado: sem prefixo
    // você pode mudar isso se quiser obrigar prefixo
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      // 1) Job via VIEW do cliente (não acessa jobs direto)
      ...
      // 2) Disputa via VIEW do cliente (nome que seu app já usa)
      //    Essa view é disputes-only e filtra opened_by_user_id = auth.uid()
      // 2) Disputa via VIEW do cliente (disputes-only)
      final disputeRes =
...
// fotos já vêm no JSON da view
  ...
  // ---------------------------------------------------------------------------
  // UPLOAD FOTOS (cliente) - mantém gravação em dispute_photos
  // ---------------------------------------------------------------------------
  ...
  Future<void> _openChat() async {
    ...
    try {
      ...
      // Chat: se job fechado, só libera se disputa OPEN
  ...
  // ---------------------------------------------------------------------------
  // AÇÃO: PROBLEMA RESOLVIDO (cliente) -> usa seu JobRepository/RPC
  // ---------------------------------------------------------------------------
  ...
  Future<void> _onProblemResolved() async {
    ...
    try {
      ...
      // mantém como você já usa (RPC / lógica no repo)
  ...
  Widget _buildActionSection({
    ...
    // Disputa open: cliente pode marcar como resolvida (via RPC)
    return Column(
