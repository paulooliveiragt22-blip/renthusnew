-- Update rpc_provider_ensure_me to accept optional p_user_id fallback.
-- auth.uid() can be null immediately after signUp (session timing), so
-- passing user.id explicitly from Flutter prevents the null user_id error.
